import { CheckCircle2, FlaskConical, PauseCircle, Trash2, XCircle } from 'lucide-react';
...
function ActiveExperiment({ experiment, days, throughDateKey, onCaptureTrial, onRemoveTrial, onFinish, onAbandon }) {
  ...
  return (
    <article className="rounded-3xl border border-indigo-100 bg-white p-5 shadow-sm">
      ...
      {eligible.length > 0 && !result.targetMet && <div className="mt-4 space-y-2"><div className="text-[10px] font-black text-gray-400">実績済み・実験登録待ち</div>{eligible.map((record) => <div key={record.recordKey} className="rounded-xl border border-dashed border-indigo-200 p-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="truncate text-xs font-bold text-gray-700">{formatShortDateLabel(record.dateKey)} {record.planTitle}</div><div className="mt-0.5 text-[9px] text-gray-400">記録上の結果: {record.observedLabel}</div></div><button type="button" onClick={() => onCaptureTrial(experiment.id, record)} className="shrink-0 rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-bold text-white">対策を試した</button></div></div>)}</div>}
