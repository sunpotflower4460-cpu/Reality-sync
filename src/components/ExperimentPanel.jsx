...
function RetentionCard({ experiment, summary, canRevalidate, todayView, onRevalidate }) {
  ...
  return (
    <article className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black ${tone}`}>{label}</span><span className="rounded-full bg-gray-100 px-2 py-1 text-[10px] font-black text-gray-500">{versionBadge(experiment)}</span></div><div className="mt-2 truncate text-xs font-extrabold text-gray-800">{experiment.title}</div></div><div className="shrink-0 text-right"><div className="text-lg font-black text-indigo-600">{summary.assessmentCount}</div><div className="text-[9px] text-gray-400">直近評価件数</div></div></div>
