...
function experimentRevision(experiment) { return JSON.stringify(experiment); }
...
function ActiveExperiment({ experiment, days, throughDateKey, onCaptureTrial, onRemoveTrial, onFinish, onAbandon }) {
  ...
  const revision = experimentRevision(experiment);
  return (
    <article className="rounded-3xl border border-indigo-100 bg-white p-5 shadow-sm">
      ...
      {experiment.trials.length > 0 && <div className="mt-4 space-y-2"><div className="text-[10px] font-black text-gray-400">登録済みの試行</div>{experiment.trials.map((trial) => <div key={trial.recordKey} className="flex items-center gap-2 rounded-xl bg-gray-50 p-2.5"><span className={`h-2.5 w-2.5 rounded-full ${trial.outcome === 'success' ? 'bg-green-500' : 'bg-orange-500'}`} /><div className="min-w-0 flex-1"><div className="truncate text-xs font-bold text-gray-700">{formatShortDateLabel(trial.dateKey)} {trial.planTitle}</div><div className="text-[9px] text-gray-400">結果: {trial.observedLabel}</div></div><button type="button" onClick={() => onRemoveTrial(experiment.id, trial.recordKey, revision)} aria-label="この試行を取り消す" className="rounded-lg p-1.5 text-gray-400 hover:bg-white"><XCircle className="h-4 w-4" /></button></div>)}</div>}

      {eligible.length > 0 && !result.targetMet && <div className="mt-4 space-y-2"><div><div className="text-[10px] font-black text-gray-400">実績済み・実験登録待ち</div><p className="mt-1 text-[9px] leading-relaxed text-gray-400">対策を実際に試した回は、良かった/悪かったに関係なく登録してください。条件付き実験では、固定した条件を満たす回だけここへ出ます。</p></div>{eligible.map((record) => <div key={record.recordKey} className="rounded-xl border border-dashed border-indigo-200 p-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="truncate text-xs font-bold text-gray-700">{formatShortDateLabel(record.dateKey)} {record.planTitle}</div><div className="mt-0.5 text-[9px] text-gray-400">この回で今回の対策を実際に試しましたか？</div></div><button type="button" onClick={() => onCaptureTrial(experiment.id, record, revision)} className="shrink-0 rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-bold text-white">対策を試した</button></div></div>)}</div>}

      {result.targetMet ? <div className="mt-4 rounded-2xl border border-gray-100 p-4"><div className="text-xs font-extrabold text-gray-800">この小実験をどう扱いますか？</div><p className="mt-1 text-[10px] leading-relaxed text-gray-400">数回の結果だけで因果確定はしません。次の計画に取り入れるかは、観測結果を見た上で明示的に決めます。</p><div className="mt-3 grid grid-cols-3 gap-2"><DecisionButton label="採用" Icon={CheckCircle2} onClick={() => onFinish(experiment.id, EXPERIMENT_DECISION.ADOPT, revision)} /><DecisionButton label="保留" Icon={PauseCircle} onClick={() => onFinish(experiment.id, EXPERIMENT_DECISION.HOLD, revision)} /><DecisionButton label="見送り" Icon={XCircle} onClick={() => onFinish(experiment.id, EXPERIMENT_DECISION.REJECT, revision)} /></div></div> : <button type="button" onClick={() => onAbandon(experiment.id, revision)} className="mt-4 w-full rounded-xl border border-gray-200 py-2.5 text-xs font-bold text-gray-500">この実験を中止する</button>}
    </article>
  );
}
...
function PastExperiment({ experiment, onDelete, hasChildren }) {
  const result = calculateExperimentResult(experiment);
  const adopted = experiment.status === EXPERIMENT_STATUS.COMPLETED && experiment.decision === EXPERIMENT_DECISION.ADOPT;
  const canDelete = !hasChildren && !adopted;
  const requestDelete = () => {
    if (!window.confirm(`「${experiment.title}」の実験履歴を削除しますか？`)) return;
    const deleted = onDelete(experiment.id, experimentRevision(experiment));
    if (deleted === false) {
      window.alert('削除直前にこの実験履歴が更新されました。古い内容から削除しないよう操作を停止しました。最新の履歴を確認してください。');
    }
  };
  return <div className="rounded-2xl border border-gray-100 bg-white p-3 shadow-sm"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="rounded-full bg-gray-100 px-2 py-1 text-[9px] font-black text-gray-500">{versionBadge(experiment)}</span><div className="truncate text-xs font-extrabold text-gray-700">{experiment.title}</div></div><div className="mt-1 text-[10px] text-gray-400">{experiment.status === EXPERIMENT_STATUS.ABANDONED ? '中止' : decisionCopy(experiment.decision)} ・ {result.trialCount}回 ・ 実験中失敗率 {percent(result.failureRate)}</div>{experiment.contextRule && <div className="mt-1 text-[9px] font-bold text-violet-500">{contextRuleLabel(experiment.contextRule)}</div>}</div>{canDelete && <button type="button" onClick={requestDelete} aria-label="実験履歴を削除" className="rounded-lg p-1.5 text-gray-300 hover:text-red-400"><Trash2 className="h-4 w-4" /></button>}</div>{hasChildren && <p className="mt-2 text-[9px] text-gray-400">後続バージョンの親履歴なので削除できません。</p>}{adopted && !hasChildren && <p className="mt-2 text-[9px] leading-relaxed text-gray-400">採用済みの学びは予定やテンプレートの適用履歴の出所になるため、個別には削除できません。すべて消す場合は設定とデータの全削除を使ってください。</p>}</div>;
}
