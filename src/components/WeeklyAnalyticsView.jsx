    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <button type="button" onClick={() => onChangeDate(shiftDateKey(selectedDate, -7))} aria-label="前の週へ" className="rounded-full bg-gray-100 p-2 text-gray-600 transition hover:bg-gray-200"><ChevronLeft className="h-5 w-5" /></button>
          <div className="min-w-0 text-center"><div className="flex items-center justify-center gap-1.5 text-sm font-extrabold text-gray-800"><CalendarDays className="h-4 w-4 text-indigo-500" />週間レビュー</div><div className="mt-1 text-xs font-medium text-gray-500">{formatWeekLabel(selectedDate)}</div></div>
          <button type="button" onClick={() => onChangeDate(shiftDateKey(selectedDate, 7))} aria-label="次の週へ" className="rounded-full bg-gray-100 p-2 text-gray-600 transition hover:bg-gray-200"><ChevronRight className="h-5 w-5" /></button>
        </div>
      </section>
      ...
      {!hasWeekData ? (
        ...
        <>
            ...
            <div className="space-y-3">
              {insights.daily.map((day) => (
                <div key={day.dateKey} className="rounded-2xl border border-gray-100 bg-gray-50 p-3">
                  <div className="mb-2 flex items-center justify-between gap-3"><div className="font-bold text-gray-700">{formatShortDateLabel(day.dateKey)}</div><div className="text-xs text-gray-500">記録 {day.recorded}/{day.total}件</div></div>
                  <div className="mb-2 h-2 overflow-hidden rounded-full bg-gray-200"><div className="h-full rounded-full bg-indigo-500" style={{ width: `${day.recordingRate}%` }} /></div>
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-gray-500"><span>理想 {formatTime(day.plannedMinutes)}</span><span>現実 {formatTime(day.actualMinutes)}</span><span className={shiftTone(day.averageStartDelta)}>開始 {formatStartShift(day.averageStartDelta)}</span></div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 className="mb-2 flex items-center gap-2 text-base font-bold text-gray-800"><Activity className="h-5 w-5 text-indigo-500" />カテゴリ別の理想 vs 現実</h2><p className="mb-5 text-xs leading-relaxed text-gray-500">週全体で、予定した時間と実際に記録した時間を比較します。</p>
            <div className="space-y-5">{Object.entries(insights.categories).map(([category, data]) => <div key={category} className="space-y-1.5"><div className="text-sm font-bold text-gray-700">{category}</div><WeeklyBar label="理想" value={data.ideal} max={maxCategoryTime} className="bg-indigo-200" /><WeeklyBar label="現実" value={data.actual} max={maxCategoryTime} className="bg-indigo-500" strong /></div>)}</div>
          </section>

          <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 className="mb-2 text-base font-bold text-gray-800">変更・スキップの理由</h2>
            {insights.reasons.length === 0 ? <p className="text-sm leading-relaxed text-gray-500">この週は理由付きの変更・スキップ記録がまだありません。</p> : <div className="space-y-2">{insights.reasons.map(({ reason, count }, index) => <div key={reason} className="flex items-start gap-3 rounded-2xl bg-orange-50 p-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-xs font-black text-orange-600 shadow-sm">{index + 1}</span><div className="min-w-0 flex-1 break-words text-sm font-medium text-gray-700">{reason}</div><span className="shrink-0 text-xs font-black text-orange-600">{count}件</span></div>)}</div>}
          </section>

          <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 className="mb-4 text-base font-bold text-gray-800">負荷と気分の観測</h2>
            <div className="grid grid-cols-3 gap-2"><OutcomeStress label="予定通り" value={insights.stressByStatus[STATUS.AS_PLANNED].average} /><OutcomeStress label="変更" value={insights.stressByStatus[STATUS.CHANGED].average} /><OutcomeStress label="スキップ" value={insights.stressByStatus[STATUS.SKIPPED].average} /></div>
            <div className="mt-4 grid grid-cols-3 gap-2"><MoodStat Icon={Smile} label="良い" value={insights.moodCounts[MOOD.GOOD]} /><MoodStat label="普通" value={insights.moodCounts[MOOD.NORMAL]} /><MoodStat Icon={Frown} label="疲れた" value={insights.moodCounts[MOOD.BAD]} /></div>
...
function OutcomeStress({ label, value }) { return <div className="rounded-2xl bg-indigo-50 p-3 text-center"><div className="text-[10px] font-bold text-indigo-400">{label}</div><div className="mt-1 text-xl font-black text-indigo-700">{stressLabel(value)}</div><div className="text-[9px] text-indigo-400">平均負荷</div></div>; }
function MoodStat({ Icon, label, value }) { return <div className="rounded-2xl bg-gray-50 p-3 text-center">{Icon ? <Icon className="mx-auto mb-1 h-4 w-4 text-gray-400" /> : <div className="mx-auto mb-1 h-4 w-4 rounded-full border-2 border-gray-300" />}<div className="text-[10px] font-bold text-gray-400">{label}</div><div className="mt-1 text-lg font-black text-gray-700">{value}</div></div>; }
