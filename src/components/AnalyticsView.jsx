  ...
  return (
    <div className="animate-fade-in space-y-5 pt-4">
      ...
      <details className="rounded-2xl border border-gray-100 bg-white shadow-sm">
        <summary className="flex min-h-12 cursor-pointer list-none items-center px-4 py-3 text-xs font-extrabold text-gray-500">週・月の振り返り</summary>
        <div className="border-t border-gray-100 p-3">
          <div className="mb-3 grid grid-cols-2 rounded-xl bg-gray-100 p-1" role="group" aria-label="振り返り期間">
            <button type="button" onClick={() => setDetailScope('week')} aria-pressed={detailScope === 'week'} className={`min-h-10 rounded-lg px-3 text-xs font-extrabold transition ${detailScope === 'week' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}>週</button>
            <button type="button" onClick={() => setDetailScope('month')} aria-pressed={detailScope === 'month'} className={`min-h-10 rounded-lg px-3 text-xs font-extrabold transition ${detailScope === 'month' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}>月</button>
          </div>
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm">
        <h2 className="flex items-center gap-2 text-base font-extrabold text-gray-800"><Layers className="h-5 w-5 text-indigo-500" aria-hidden="true" />理想の軌跡 vs 現実の歩み</h2>
        <p className="mt-1.5 text-[11px] leading-relaxed text-gray-500">予定外の行動も、実際に記録した時間だけを「現実」として比べます。</p>
          ...
          {Object.entries(stats.categories).map(([category, data]) => {
            ...
            return (
              <div key={category} className="space-y-1.5">
                <div className="flex items-center gap-2"><div className="truncate text-sm font-extrabold text-gray-700">{category}</div>{unexpectedGain && <span className="flex shrink-0 items-center gap-0.5 rounded-full bg-pink-100 px-1.5 py-0.5 text-[9px] font-bold text-pink-600"><Sparkles className="h-3 w-3" />予定外</span>}</div>
      ...
      <section className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="flex items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-base font-extrabold text-gray-800"><Activity className="h-5 w-5 text-indigo-500" aria-hidden="true" />予定の達成度サマリー</h2><p className="mt-1 text-[10px] text-gray-400">うまくいかなかった日も、次の計画の材料です</p></div><span className="shrink-0 rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-black text-indigo-600">{stats.completionRate}%</span></div>
        <div className="mt-4 flex h-10 overflow-hidden rounded-xl border border-gray-100 shadow-inner">
          ...
          <Segment width={stats.pending / Math.max(stats.total, 1)} className="bg-gray-100" label={stats.pending > 0 ? '未' : ''} muted />
        </div>
        <div className="mt-3 grid grid-cols-2 gap-2.5 text-sm"><SummaryCard label="予定通り" value={stats.completed} tone="text-green-600" /><SummaryCard label="変更・休み" value={stats.changed + stats.skipped} tone="text-orange-600" /></div>
      </section>
    </div>
  );
}
...
function Segment({ width, className, label, muted = false }) { return <div style={{ width: `${width * 100}%` }} className={`flex h-full items-center justify-center transition-all ${className}`}>{label && <span className={`px-1 text-[10px] ${muted ? 'font-medium text-gray-400' : 'font-bold text-white'}`}>{label}</span>}</div>; }
function SummaryCard({ label, value, tone }) { return <div className="rounded-2xl bg-gray-50 p-3 text-center"><div className="mb-1 text-[10px] font-bold text-gray-400">{label}</div><div className={`text-2xl font-black ${tone}`}>{value}<span className="ml-1 text-xs font-medium text-gray-400">件</span></div></div>; }
