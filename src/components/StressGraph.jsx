...
function createTimeScale(schedules) {
  const times = schedules.flatMap((schedule) => [schedule.time, schedule.actualStartTime].filter(Boolean));
  const hasEarlyTime = times.some((time) => timeToHours(time) < DEFAULT_START_HOUR);
  const startHour = hasEarlyTime ? 0 : DEFAULT_START_HOUR;
  const gridHours = [];
...
export function StressGraph({ schedules }) {
  ...
  const actualTimed = recorded
    .filter((schedule) => schedule.status !== STATUS.SKIPPED && schedule.actualStartTime)
    .sort((a, b) => timeToHours(a.actualStartTime) - timeToHours(b.actualStartTime));
  const actualPoints = actualTimed.map((schedule) => ({ x: getX(schedule.actualStartTime), y: getY(schedule.actualStress) }));
  ...
  const untimedCount = recorded.filter((schedule) => schedule.status !== STATUS.SKIPPED && !schedule.actualStartTime).length;
  ...
  return (
    <section className="relative mb-6 overflow-hidden rounded-3xl border border-gray-100 bg-white p-5 shadow-sm" aria-labelledby="stress-graph-title">
      ...
      <div className="mb-4 flex flex-col">
        ...
        <div className="flex gap-4 text-[10px] font-bold"><span className="flex items-center gap-1 text-gray-400"><span className="w-3 border-b border-dashed border-gray-400" />計画時刻</span><span className="flex items-center gap-1 text-indigo-600"><span className="h-1 w-3 rounded bg-indigo-500" />実際の開始時刻</span></div>
        {untimedCount > 0 && <p className="mt-1.5 text-[9px] leading-relaxed text-gray-400">開始時刻が未記録の実績 {untimedCount}件は、現実の時間軸には推測で配置していません。</p>}
      </div>
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: `${WIDTH}/${HEIGHT}` }}>
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-full w-full" role="img" aria-label="計画時刻の想定負荷と、記録された実際の開始時刻における負荷を比較する折れ線グラフ">
          ...
          {gridHours.map((hour) => { const x = PADDING_X + ((hour - startHour) / (END_HOUR - startHour)) * (WIDTH - PADDING_X * 2); return <g key={hour}><line x1={x} y1={PADDING_Y - 10} x2={x} y2={HEIGHT - PADDING_Y + 10} stroke="#f3f4f6" strokeWidth="2" strokeDasharray="4 4" /><text x={x} y={HEIGHT - 18} textAnchor="middle" fill="#9ca3af" fontSize="20" fontWeight="700">{hour === 24 ? '0' : hour}</text></g>; })}
          ...
          {actualPoints.map((point, index) => <circle key={`actual-${actualTimed[index].id}`} cx={point.x} cy={point.y} r="8" fill="#fff" stroke={actualTimed[index].actualStress > 80 ? '#ef4444' : '#6366f1'} strokeWidth="4" />)}
        </svg>
      </div>
    </section>
  );
