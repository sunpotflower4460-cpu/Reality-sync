...
export function InsightCandidatesView({ insights, experiments = [], days = {}, selectedDate, onStartExperiment, onCaptureTrial, onRemoveTrial, onFinishExperiment, onAbandonExperiment, onDeleteExperiment }) {
  ...
  const canStartToday = isToday(selectedDate);

  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-indigo-100 bg-white shadow-sm">
        <div className="bg-indigo-600 p-5 text-white"><div className="flex items-center gap-2"><FlaskConical className="h-5 w-5" /><h2 className="font-extrabold">観測インサイト</h2></div><p className="mt-2 text-xs leading-relaxed text-indigo-100">答えを断定する場所ではなく、次に確かめる価値があるパターンを、根拠と不確実性つきで並べます。</p></div>
        <div className="p-5">
...
function CandidateCard({ candidate, experimentActive, canStartToday, onExperiment }) {
  const experimentable = canCreateExperiment(candidate);
  const disabled = experimentActive || !canStartToday;
  return <article className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black ${EVIDENCE_STYLE[candidate.evidence]}`}>{candidate.evidenceLabel}</span><h3 className="mt-2 break-words text-base font-extrabold leading-snug text-gray-800">{candidate.title}</h3></div><div className="shrink-0 text-right"><div className="text-[10px] font-bold text-gray-400">観測</div><div className="text-lg font-black text-indigo-600">{candidate.sampleCount}</div><div className="text-[9px] text-gray-400">{candidate.monthCount}か月</div></div></div><div className="mt-4 space-y-3"><EvidenceBlock label="観測事実" text={candidate.observation} /><EvidenceBlock label="差と不確実性" text={candidate.comparison} /><EvidenceBlock label="仮説" text={candidate.hypothesis} emphasis /></div>{experimentable && <button type="button" disabled={disabled} onClick={onExperiment} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3 text-xs font-bold text-white disabled:bg-gray-300"><FlaskConical className="h-4 w-4" />{experimentActive ? 'この候補を実験中' : !canStartToday ? '今日の傾向で再確認して開始' : 'この候補を小さく試す'}</button>}<p className="mt-4 border-t border-gray-100 pt-3 text-[10px] leading-relaxed text-gray-400">{candidate.caution}</p></article>;
}
