...
export function SettingsModal({
  ...
  const requestNotifications = async () => {
    setMessage(''); setError('');
    if (typeof Notification === 'undefined') { setNotificationPermission('unsupported'); setError('このブラウザはWeb通知に対応していません。アプリ内の記録待ちは利用できます。'); return; }
    let permission;
  ...
  return (
    <ModalDialog onClose={onClose} labelledBy="settings-modal-title" className="max-h-[92vh] w-full max-w-sm overflow-y-auto rounded-3xl bg-white shadow-2xl">
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-100 bg-white p-4"><div><h2 id="settings-modal-title" className="font-extrabold text-gray-800">設定とデータ</h2><p className="mt-0.5 text-[10px] text-gray-400">端末内データの保全と記録忘れ対策</p></div><button type="button" onClick={onClose} aria-label="設定を閉じる" className="rounded-full bg-gray-100 p-1.5 text-gray-400 hover:text-gray-600"><XCircle className="h-5 w-5" /></button></div>
      <div className="space-y-5 p-5">
        {storageBlocked && <section className="flex items-start gap-2 rounded-2xl border border-red-200 bg-red-50 p-4 text-xs leading-relaxed text-red-700"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><div><div className="font-extrabold">保存データ保護モード</div><p className="mt-1">現在の保存データを安全に解釈できないため、自動保存とバックアップ書き出しを止めています。localStorageの元データは上書きしていません。{storageProtection.unsupportedVersion !== null ? ` 検出した保存版: ${String(storageProtection.unsupportedVersion)}` : ''}</p></div></section>}
        ...
        <section className="rounded-2xl border border-gray-100 p-4">
          ...
          <p className="mb-3 text-[10px] leading-relaxed text-gray-500">予定・実績・テンプレート・小さな実験・リマインダー設定を1つのJSONに保存します。実験では「対策を試した」と明示登録した回の結果スナップショットも含まれます。JSON自体は暗号化されません。</p>
          <div className="grid grid-cols-2 gap-2"><button type="button" onClick={exportBackup} disabled={storageBlocked} className="flex items-center justify-center gap-1.5 rounded-xl bg-indigo-600 py-3 text-xs font-bold text-white disabled:cursor-not-allowed disabled:bg-gray-300"><Download className="h-4 w-4" />書き出す</button><button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center justify-center gap-1.5 rounded-xl border border-indigo-200 bg-white py-3 text-xs font-bold text-indigo-600"><Upload className="h-4 w-4" />復元する</button></div>
          <input ref={fileInputRef} type="file" accept="application/json,.json" onChange={importBackup} className="hidden" />
        </section>

        <section className="rounded-2xl border border-gray-100 p-4"><div className="mb-2 flex items-center gap-2"><Smartphone className="h-5 w-5 text-indigo-500" /><h3 className="font-bold text-gray-800">アプリとして使う</h3></div>{isInstalled ? <p className="text-xs font-medium text-green-600">この端末ではスタンドアロン表示で起動しています。</p> : canInstall ? <button type="button" onClick={onInstall} className="mt-2 w-full rounded-xl bg-gray-900 py-3 text-xs font-bold text-white">この端末にインストール</button> : <p className="text-[10px] leading-relaxed text-gray-500">対応ブラウザではブラウザメニューの「ホーム画面に追加」「アプリをインストール」から追加できます。iPhoneでは共有メニューからホーム画面へ追加できます。</p>}</section>
