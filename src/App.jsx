  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 text-gray-800">
      ...
      <main className="mx-auto -mt-2 max-w-md p-4">
        ...
        {activeTab === TABS.TRACK && <TrackView schedules={schedules} onRecord={setSelectedSchedule} />}
        {activeTab === TABS.ANALYTICS && <AnalyticsView stats={stats} />}
      </main>

      <BottomNav activeTab={activeTab} onChange={setActiveTab} />
      {selectedSchedule && <RecordModal schedule={selectedSchedule} onClose={() => setSelectedSchedule(null)} onSave={saveRecord} />}

      {isPlanModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && setIsPlanModalOpen(false)}>
          <div role="dialog" aria-modal="true" aria-labelledby="plan-modal-title" className="w-full max-w-sm rounded-3xl bg-white p-6 text-center shadow-2xl">
