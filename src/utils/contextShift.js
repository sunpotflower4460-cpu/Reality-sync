...
function completeDayContext(experiment, days, dateKey) {
  ...
  // Full-day metrics are only used when every remaining plan has immutable historical plan evidence.
...
function numericCandidate({ id, label, previous, recent, threshold, unit, digits = 0, note = '' }) {
  if (
    previous.count < CONTEXT_SHIFT_MIN_SAMPLES
    || recent.count < CONTEXT_SHIFT_MIN_SAMPLES
    || previous.value === null
    || recent.value === null
  ) return null;
  ...
  return {
    ...
    valueKind: 'number',
    unit,
    previousValue: round(previous.value, digits),
    recentValue: round(recent.value, digits),
    difference: round(difference, digits),
    previousSampleCount: previous.count,
    recentSampleCount: recent.count,
    score: Math.abs(difference) / threshold,
    note,
  };
}
...
function candidateList(experiment, days, previousUsages, recentUsages) {
  ...
  push(numericCandidate({
    id: 'target-planned-stress', label: '対象予定の想定負荷',
    previous: mean(previous.targets.map((item) => item.plannedStress)),
    recent: mean(recent.targets.map((item) => item.plannedStress)),
    threshold: 10, unit: 'pt', note: '記録時に固定された対象予定のplannedSnapshotだけを比較',
  }));
  push(numericCandidate({
    id: 'target-duration', label: '対象予定の長さ',
    previous: mean(previous.targets.map((item) => item.plannedDuration)),
    recent: mean(recent.targets.map((item) => item.plannedDuration)),
    threshold: 15, unit: '分', note: '記録時に固定された対象予定のplannedSnapshotだけを比較',
  }));
  push(numericCandidate({
    id: 'actual-stress', label: '実際の負荷',
    previous: mean(previous.targets.map((item) => item.actualStress)),
    recent: mean(recent.targets.map((item) => item.actualStress)),
    threshold: 10, unit: 'pt', note: 'actualStressが明示記録されている回だけを比較',
  }));
  push(rateCandidate({
    id: 'bad-mood-rate', label: '「気分が悪い」記録の割合',
    previous: rate(previous.targets.map((item) => item.mood === null ? null : item.mood === MOOD.BAD)),
    recent: rate(recent.targets.map((item) => item.mood === null ? null : item.mood === MOOD.BAD)),
    note: 'moodが明示記録されている回だけを比較',
  }));
  push(numericCandidate({
    id: 'day-planned-minutes', label: '1日の予定時間',
    previous: mean(previous.days.map((item) => item.totalMinutes)),
    recent: mean(recent.days.map((item) => item.totalMinutes)),
    threshold: 60, unit: '分', note: '当日の全予定にimmutableなplannedSnapshotがある日だけ。実験の調整バッファは除外',
  }));
  push(numericCandidate({
    id: 'day-plan-count', label: '1日の予定件数',
    previous: mean(previous.days.map((item) => item.planCount)),
    recent: mean(recent.days.map((item) => item.planCount)),
    threshold: 2, unit: '件', digits: 1, note: '当日の全予定にimmutableなplannedSnapshotがある日だけ。実験の調整バッファは除外',
  }));
  push(numericCandidate({
    id: 'day-planned-stress', label: '1日の平均想定負荷',
    previous: mean(previous.days.map((item) => item.averagePlannedStress)),
    recent: mean(recent.days.map((item) => item.averagePlannedStress)),
    threshold: 10, unit: 'pt', note: '当日の全予定にimmutableなplannedSnapshotがある日だけ',
  }));

  CATEGORIES.forEach((category) => {
    push(rateCandidate({
      id: `day-category-share-${category}`,
      label: `1日の「${category}」予定時間比率`,
      previous: mean(previous.days.map((item) => item.categoryShares[category])),
      recent: mean(recent.days.map((item) => item.categoryShares[category])),
      note: '当日の全予定にimmutableなplannedSnapshotがある日だけ。予定時間に占める比率を比較',
    }));
  });
...
export function calculateContextShiftSummary(experimentValue, days, throughDateKey) {
  ...
  if (previousWindow.count < RETENTION_MIN_USES || previousWindow.weekCount < RETENTION_MIN_WEEKS) {
    return {
      ...
      available: false,
      reason: `悪化前の通常運用が${RETENTION_MIN_USES}件以上・${RETENTION_MIN_WEEKS}週以上ないため、Context Shiftを比較しません。`,
      previousWindow,
      recentWindow,
      candidates: [],
    };
  ...
  if (
    retention.experimentFailureRate === null
    || previousWindow.failureRate === null
    || previousWindow.failureRate - retention.experimentFailureRate > 0.1
  ) {
    return {
      experimentId: experiment.id,
      available: false,
      reason: '悪化前の比較窓も実験中よりすでに大きく悪化していたため、「以前は安定していた期間」とはみなさず条件差を推測しません。',
      previousWindow,
      recentWindow,
      candidates: [],
    };
  ...
  return {
    ...
    reason: candidates.length > 0
      ? '以前の通常運用期と直近悪化期で、同時に変わっていた明示記録上の条件です。原因を示すものではありません。'
      : '比較できる記録の中では、設定した最小差を超えるContext Shift候補は見つかりませんでした。',
    previousWindow,
    recentWindow,
    candidates,
  };
