...
test('timeline uses calm white cards with status accents rather than full tinted status panels', () => {
  const text = source('src/components/TrackView.jsx');
  assert.match(text, /app-card relative w-full/);
  assert.match(text, /absolute inset-y-3 left-0 w-\[3px\]/);
...
test('analytics makes completion and ideal-versus-reality the primary commercial reading order', () => {
  const text = source('src/components/AnalyticsView.jsx');
  const completion = text.indexOf('予定の達成度サマリー');
  const comparison = text.indexOf('理想の軌跡 vs 現実の歩み');
...
test('commercial shell stays visually refined without adding navigation beyond the three core tabs', () => {
  ...
  assert.match(app, /bg-gradient-to-br from-indigo-600/);
